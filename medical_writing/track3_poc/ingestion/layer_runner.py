...
class LayerRunner:
    ...
    def commit_layer_verifications(
        ...
        for record in verification_records:
            ...
            if decision == VerificationDecision.CONFIRMED:
                # Check if this is an inherited node
                if getattr(record, 'is_inherited', False):
                    final_val = record.inherited_value
                    session.confirmed_values[record.node_id] = final_val
                    # Don't increment total_nodes_confirmed for inherited (already counted in total_nodes_inherited)
                    if twin:
                        twin.set(record.node_id, final_val,
                                 source=f"inherited:{session.parent_twin_id or 'parent'}",
    ...
    def auto_verify_layer(
        ...
        for i, node in enumerate(nodes):
            ...
            inherited_value = getattr(session, 'inherited_values', {}).get(node.id)
            is_inherited = inherited_value is not None
            record = NodeVerificationRecord(
                ...
                extracted_items=[] if is_inherited else [extracted_item],
                decisions=[VerificationDecision.CONFIRMED],
                corrected_values=[inherited_value if is_inherited else None],
                override_justifications=[None],
                final_value=inherited_value if is_inherited else extracted_item.value,
                final_status="inherited" if is_inherited else "confirmed",
